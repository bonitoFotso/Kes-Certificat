// src/components/certificates/HeightWorkFileUploader.tsx
...
interface HeightWorkFileUploaderProps {
  onFilesProcessed?: (successCount: number, errorCount: number) => void;
}

export const HeightWorkFileUploader: React.FC<HeightWorkFileUploaderProps> = ({ 
  onFilesProcessed 
}) => {
  ...
  // Réinitialise les compteurs lorsque les fichiers changent
  useEffect(() => {
    ...
    for (let i = 0; i < files.length; i++) {
      if (files[i].status === FileStatus.PENDING) {
        ...
        try {
          ...
          const result = await certificateService.generateHeightWorkCertificate(
            files[i].file,
    ...
    <div className="max-w-2xl mx-auto p-6 space-y-6">
      ...
      >
        <input
          ...
          className="hidden"
          id="file-upload-height-work"
        />

        <label
          htmlFor="file-upload-height-work"
...
export default HeightWorkFileUploader;